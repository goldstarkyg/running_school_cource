...
@Injectable()
export class PendingTrainerListService implements Resolve<any>
{
    public PENDING_TRAINER_LIST_URL = `${isDevMode() && env.baseUrl || envProd.baseUrl}school/personal`;
    ...
    getPendingTrainers(): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            this._httpClient.post(this.PENDING_TRAINER_LIST_URL, data)
                .subscribe((response: any) => {
                    this.pendingTrainers = response;
                    resolve(response);
                }, reject);
        }
        );
