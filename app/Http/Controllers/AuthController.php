        ...
        try {
            ...
            if (Sentinel::authenticate($request->only(['email', 'password']), $request->get('remember-me', false))) {
                ...
                if($role_id == '1')
                    ...
                    return Redirect::route("dashboard_super")->with('success', trans('auth/message.signin.success'));
                if($role_id == '2')
                    return Redirect::route("dashboard_school")->with('success', trans('auth/message.signin.success'));
                if($role_id == '3' || $role_id == '4')
                    return Redirect::route("dashboard_personal")->with('success', trans('auth/message.signin.success'));
                if($role_id == '5')
                    return Redirect::route("dashboard_athlete")->with('success', trans('auth/message.signin.success'));
            }
