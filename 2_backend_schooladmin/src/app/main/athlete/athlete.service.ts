...
{
    // 'school/course/list/athlete/{school_id}
    public ATHLETELISTBYID_URL = `${isDevMode() && env.baseUrl || envProd.baseUrl}school/course/list/athlete/`;
    ...
    getAthletesBySchool(): Promise<any> {
        let data: any = {};
        data['token'] = this._localSession.retrieveToken();
        data['user_id'] = this._localSession.retrieveUserID();
        data['status'] = this.dispMode.id;

        let school_id = this._localSession.retrieveSchoolID();
        return new Promise((resolve, reject) => {
            this._httpClient.post(this.ATHLETELISTBYID_URL + school_id, data)
