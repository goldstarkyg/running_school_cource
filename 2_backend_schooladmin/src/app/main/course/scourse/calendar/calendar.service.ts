...
//[{"lesson_name":"test aaa title","lesson_desc":"desc ttt","lesson_date":"2019-07-02","start_time":"09:00","end_time":"03:00"}]
...
export class CalendarService implements Resolve<any>
{
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any
    {
        ...
        this.lv_id = route.params.lvid;
        this.tr_id = route.params.trid;

        return new Promise((resolve, reject) => {
            Promise.all([ this.getCourseContent() ]).then( ([events]: [any]) => { resolve(); }, reject );
        });
    }
    ...
    updateEvents(events): Promise<any>
    {
        ...
        data['token']       = this._localSession.retrieveToken();
        data['school_id']   = this._localSession.retrieveSchoolID();
        data['level_id']    = this.lv_id;
        data['trainer_id']  = this.tr_id;
